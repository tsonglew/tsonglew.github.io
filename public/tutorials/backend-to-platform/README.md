...
- [打开完整提示词](./prompts.md)，八份委托涵盖工程、界面、需求、架构、排错与性能。
- [复制交付工作簿](./workbook.md)，记录六课的决策、修改和真实证据。
...
- `http://127.0.0.1:8080/workbook.md`，六课交付记录。
...
5. 把复现步骤、预期、实际和完整时间线填进 `prompts.md` 的“05 证据排错 prompt”，结果记在 `workbook.md` 中。
...
5. 用 DevTools Performance 录制定位 `computeRange`，把真实证据填进“06 性能优化 prompt”。
...
先复制 [workbook.md](./workbook.md)，每一课都在同一项目中继续记录。

| 课程        | 从哪些材料开始               | 本课留下什么              |
| ----------- | ---------------------------- | ------------------------- |
| 01 工程接手 | README、页面入口、实验脚本   | 工程地图与运行记录        |
| 02 界面质量 | 静态布局基线、真实截图与尺寸 | 视觉约束和布局验收        |
| 03 需求开发 | 前两课记录、resources.json   | 在 prototype 中生成的原型 |
| 04 架构选型 | 原型、已有工程与真实部署约束 | ADR 和状态归属            |
| 05 证据排错 | 实验室日志与自己的复现       | 原因、修复和回归证据      |
| 06 性能调优 | 同一版本、数据规模和环境     | 原始测量、收益与代价      |

`prompts.md` 收录八份主要委托，前两课各有两份。下载包没有现成 `prototype`，第三课由读者委托 AI 实现。真实 API、会话和线上部署属于后续实践，在工作簿中另行标记，不能用模拟结果提前验收。

## 文件与复核

```text
index.html            AI 实战工具箱入口
prompts.md            六课的八套完整提示词
workbook.md           工程、界面、架构、排错与性能记录
